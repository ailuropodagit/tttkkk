...
class Ion_auth_model extends CI_Model
{
    ...
    public function login($identity, $password, $remember = FALSE, $main_groupid = NULL)
    {
        ...
        {
            $main_groupid = 0;
        } //if don't have main group id pass in, then this login can login to any group
    ...
    public function set_session($user)
    {
        ...
        $session_data = array(
            ...
            'username' => $user_extrainfo->username,
    ...
    {
        $this->trigger_events('get_main_category_list');
     ...
     * @return array
     * */
    public function get_sub_category_list($option_type = NULL)
    {
        $this->trigger_events('get_sub_category_list');
        $this->db->from('category')
                ->where('category_level', 1)
                ->where('main_category_id', $option_type)
                ->order_by('category_label');
        $result = $this->db->get();
        $return = array();
        if ($result->num_rows() > 0)
